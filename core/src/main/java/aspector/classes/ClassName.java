...
import static endfield.util.Strings2.substringAfter;
import static endfield.util.Strings2.substringAfterLast;
import static endfield.util.Strings2.substringBeforeLast;
import static endfield.util.Strings2.trimEnd;
...
public class ClassName {
	...
	public static String descToInternal(String signatureName) {
		return switch (signatureName.charAt(0)) {
			...
			case 'F' -> "float";
			case 'D' -> "double";
			case 'L' -> trimEnd(signatureName.substring(1), ';');
	...
	public ClassName componentName() {
		return new ClassName(substringAfter(descriptor, "["));
	}
