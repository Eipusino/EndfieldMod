...
public class MethodSignature {
	static final Pattern namedSignatureMatcher = Pattern.compile("^[^.;()]+\\(([BCDFIJSZ]|\\[+[BCDFIJSZ]|\\[*L[^;]+;)*\\)([BCDFIJSZ]|\\[+[BCDFIJSZ]|V|\\[*L[^;]+;)$");
	static final Pattern signatureMatcher = Pattern.compile("^\\(([BCDFIJSZ]|\\[+[BCDFIJSZ]|\\[*L[^;]+;)*\\)([BCDFIJSZ]|\\[+[BCDFIJSZ]|V|\\[*L[^;]+;)$");
	...
	public static MethodSignature parse(String methodName, String signature) {
		if (!signatureMatcher.matcher(signature).matches())
	...
	public static MethodSignature parse(String signature) {
		if (!namedSignatureMatcher.matcher(signature).matches())
			throw new IllegalArgumentException("Invalid signature string: " + signature);
