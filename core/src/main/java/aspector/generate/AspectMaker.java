...
import static endfield.util.Arrays2.associateBy;
...
public final class AspectMaker extends ClassMaker {
	...
	public ClassName generateClassName(ClassDecl<?> aspectImpl, ClassDecl<?> targetClass) {
		...
		return ClassName.byName(target.startsWith("java.") ? target.replace("java.", "javas.") : target
				+ "$" + aspectImpl.name.simpleName()
				+ "@" + Integer.toHexString(aspectImpl.name.hashCode()));
	}
	...
	public byte[] generateBytecode(AspectBuilder builder) {
		...
		var fields = filterIsInstance(elements, EField.class);
		...
		Map<MethodSignature, MethodNode> implMethods = associateBy(implRoot.methods, node -> MethodSignature.parse(node.name, node.desc));
		...
		cw.visit(
				...
				map(interfaces, ClassName::internalName).toArray(Constant.EMPTY_STRING)
		);
		...
		for (EAspectMethod e : aspectElements) {
			...
			if (byMethod != null) {
				var bridge = switch (e.using) {
					case OVERRIDE -> new MethodSignature("NONE", List.of(), ClassName.V);
					default -> buildBridgeMethod(
							cw,
							byMethod,
							thisClass,
							superClass,
