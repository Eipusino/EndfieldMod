...
public abstract class AnnotationValue<T, R> {
	...
	public static final class Value<T> extends AnnotationValue<T, T> {
	...
	public static final class TypeValue extends AnnotationValue<Class<?>, ClassName> {
	...
	public static final class ArrayValue<V, T extends AnnotationValue<V, ?>> extends AnnotationValue<List<V>, List<T>> {
		public final List<V> value;
		public final List<T> rawValue;

		public ArrayValue(List<T> values) {
			value = CollectionsKt.map(values, t -> t.value());
			rawValue = values;
		}

		@Override
		public ClassName getType() {
			return ClassName.CLASS.arrayName();
		}
	...
	public static final class NestedAnnotationValue extends AnnotationValue<Annotation, EAnnotation> {
			...
			if (value == null) {
				try {
					...
					value = (Annotation) java.lang.reflect.Proxy.newProxyInstance(
							...
							new Class[]{annoType},
							(obj, method, args) -> {
								AnnotationValue<?, ?> value = rawValue().getValue(method.getName());
								return value != null ? value.value() : method.invoke(obj, args);
							}
					);
				} catch (ClassNotFoundException e) {
					throw new RuntimeException(e);
	...
	public static final class EnumValue<T extends Enum<T>> extends AnnotationValue<T, Pair<ClassName, String>> {
