		...
		for (int i = 1; i < Vars.maxBlockSize; i++) {
			...
			for (int j = 0; j < 4; j++) {
				...
				for (int m = minOff; m <= rad; m++) {
					edges[i][j][m + rad - off] = switch (j) {
						case 0 -> new Point2(rad + 1, m);
						case 1 -> new Point2(m, rad + 1);
						case 2 -> new Point2(-rad - 1 + off, m);
						case 3 -> new Point2(m, -rad - 1 + off);
						default -> new Point2();
					};
				}

				angle[i][j][0] = switch (j) {
