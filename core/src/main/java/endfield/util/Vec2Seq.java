 ...
 * <p>Similar implemented in {@link mindustry.graphics.Trail};
 *
 * @see FloatSeq
 * @see Vec2
 */
public class Vec2Seq implements Iterable<Vec2>, Eachable<Vec2> {
	...
	public void addAll(Vec2Seq vec2s) {
		for (int i = 0; i < vec2s.size(); i++) {
			coordinates.addAll(vec2s.coordinates.items[i * 2], vec2s.coordinates.items[i * 2 + 1]);
		}
	}
