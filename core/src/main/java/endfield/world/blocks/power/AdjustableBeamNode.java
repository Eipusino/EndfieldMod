...
public class AdjustableBeamNode extends BeamNode {
	...
	public byte[][] beamDirections = {{1, 0}, {0, 1}, {-1, 0}, {0, -1}};
	...
	public void drawPlace(int x, int y, int rotation, boolean valid) {
		for (byte[] beamDirection : beamDirections) {
			int maxLen = range + size / 2;
			Building dest = null;
			Point2 dir = new Point2(beamDirection[0], beamDirection[1]).rotate(rotation);
	...
	public class AdjustableBeamNodeBuild extends BeamNodeBuild {
		...
		public void updateDirections() {
			for (int i = 0; i < beamDirections.length; i++) {
				...
				Point2 dir = new Point2(beamDirections[i][0], beamDirections[i][1]).rotate(rotation);
				links[i] = null;
				dests[i] = null;
				int offset = size / 2;
