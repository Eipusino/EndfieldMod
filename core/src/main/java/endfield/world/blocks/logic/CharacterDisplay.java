...
public class CharacterDisplay extends Block {
	...
	public CharacterDisplay(String name) {
		...
		config(Point2.class, (CharacterDisplayBuild build, Point2 packed) -> {
			build.displayColor = packed.x;
			build.displayCharacter = packed.y;
		});
		configClear((CharacterDisplayBuild build) -> {
	...
	public void placeEnded(Tile tile, Unit builder, int rotation, Object config) {
		if (tile.build != null && !queueText.isEmpty()) {
			if (config instanceof Point2 point) {
				point.x = CharacterOverlay.charToData(queueText.charAt(0));
				tile.build.configure(new Point2(point));
			} else if (config instanceof int[] conf && conf.length == 2) {
	...
	public class CharacterDisplayBuild extends Building {
		...
		public Point2 config = new Point2();
		...
		public void buildConfiguration(Table table) {
			...
			color.table(input -> {
				...
				input.button(Icon.pick, Styles.clearNonei, () -> Vars.ui.picker.show(Tmp.c1.set(displayColor), c -> configure(config.set(c.rgba(), displayCharacter))));
			}).growX().row();
			...
			for (int region = 0; region < letterRegions.length; region++) {
				...
				button.changed(() -> configure(config.set(displayColor, button.isChecked() ? r : -1)));
		...
		public Object config() {
			return config.set(displayColor, displayCharacter);
		}
