...
public final class PatternManager {
		...
		runningThread = Threads.thread("Pattern-Resolver", () -> {
			...
			for (Tile tile : dirty) {
				for (Patterned p : getPatternedBlocks(tile)) {
					...
					IntSetIterator iter = contiguous.iterator();
					while (iter.hasNext) {
						CollectionObjectMap<Block, Tile> map = tileToAnchorMapCopy.get(iter.next());
						if (map != null) {
			...
			for (PatternAnchor anchor : toRemove) {
			...
			Core.app.post(() -> {
				...
				IntSetIterator iter = toResolve.iterator();
				while (iter.hasNext) {
					Tile tile = Vars.world.tile(iter.next());
					if (tile != null) Vars.renderer.blocks.floor.recacheTile(tile);
				}
	...
	static void resolve(IntSet toResolve, IntMap2<CollectionObjectSet<Block>> resolved, Cons<PatternAnchor> onPatternFound) {
		...
		for (int i = 0; i < sortedToResolve.size; i++) {
			int pos = sortedToResolve.items[i];
