...
import endfield.ui.dialogs.GameDataDialog;
import endfield.ui.dialogs.PowerGraphInfoDialog;
...
public final class Elements {
	...
	public static Element[] sliderSet(Table table, Cons<String> changed, Prov<String> fieldText, TextFieldFilter filter, TextFieldValidator valid, float min, float max, float step, float def, SliderChanged sliderChanged, String title, @Nullable String tooltip) {
		...
		Slider sl = table.slider(min, max, step, def, s -> sliderChanged.get(s, field)).right().width(Vars2.sliderWidth).get();
		TextField f = table.add(field).left().padLeft(6f).width(Vars2.fieldWidth).get();
